...
pub fn read_input_lines(allo: *std.mem.Allocator, inp_fn: []const u8) ![][]const u8 {
    ...
    var byte_buffer = try allo.alloc(u8, 2048);
    defer allo.free(byte_buffer);

    const nr_bytes = try f.readAll(byte_buffer);
    var lines = std.mem.split(byte_buffer[0..nr_bytes], &separator);

    var out_lines: [][]const u8 = try allo.alloc([]const u8, 1024);
